...
- `W[0..K-1]` — integer width schedule from a **fixed, model-agnostic analytic
  curve generated by integer-only arithmetic** (no floating point anywhere in
  its construction). This is deliberate: a curve computed from a float formula
  (e.g. `pow`) could yield *different integer constants on different platforms*
  and reintroduce the divergence we are eliminating. Candidate family — a
  geometric decay via a rational-ratio integer recurrence
  `w[i] = max(1, w[i-1] * num // den)` (e.g. `num/den = 3/4`), or an integer Zipf
  `w[i] = max(1, BASE // (i + 1) ** s)`. The decay steepness (the parameter that
  trades capacity against text naturalness) is tuned during implementation and
  then **frozen** as part of the constant.
- `CUM[0..K]` — prefix sums of `W` scaled to exactly `2³² = WHOLE` using
  integer multiply + floor-divide only, with `CUM[0] = 0`, `CUM[K] = WHOLE`, and
  each interval forced to width ≥ 1 (same min-width rule as today).
...
computed from floats at runtime.** They may be generated once at import time
*provided the generation uses only integer arithmetic* (Python `int` is
arbitrary-precision and platform-independent), or frozen as literals.
...
Widths come from rank via a fixed analytic curve, not the true distribution, so:

- **Capacity:** bits/token ≈ the entropy of the `W` schedule. The curve's
  steepness is the lever — flatter = more bits/token but more low-probability
  tokens forced into the text; steeper = more natural text but lower capacity.
  Tuned and **measured** (new vs old bits/token) during implementation and
  reported.
- **Quality:** tokens are sampled per the fixed schedule rather than the exact
  model distribution; near-ties resolve to the lowest `token_id`. A moderately
  steep curve keeps high-rank (high-probability) tokens dominant so prose stays
  natural; near-ties are cosmetically irrelevant. Spot-checked during
  implementation.
...
- Exact analytic curve family and steepness for `W[]` (integer-only generation
  is required; the parameter choice trades capacity vs naturalness) and the
  resulting bits/token.
